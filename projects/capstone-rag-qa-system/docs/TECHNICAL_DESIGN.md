...
│   │                RAGEvaluator                           │  │
│   │                                                       │  │
│   │  · load_dataset()     加载评估数据集                  │  │
│   │  · run_evaluation()   执行 Ragas 评估                 │  │
│   │  · generate_report()  生成评估报告                    │  │
│   └──────────┬───────────────────────┬───────────────────┘  │
│              │                       │                       │
│              ▼                       ▼                       │
│   ┌──────────────────┐   ┌──────────────────────────────┐   │
│   │  ChatService     │   │  LangChain Adapters           │   │
│   │  (复用真实管道)   │   │                               │   │
│   │  · retrieve()    │   │  ┌────────────────────────┐   │   │
│   │  · generate()    │   │  │ OllamaLangChainLLM    │   │   │
│   │                  │   │  │ (本地 Ollama 模型)     │   │   │
│   └──────────────────┘   │  ├────────────────────────┤   │   │
│                          │  │ OnlineLangChainLLM     │   │   │
│                          │  │ (OpenAI / DashScope)   │   │   │
...
| 指标               | 维度   | 说明                     |
| ------------------ | ------ | ------------------------ |
| Faithfulness       | 生成   | 回答是否忠实于检索上下文 |
| Answer Relevancy   | 生成   | 回答与问题的相关度       |
| Context Precision  | 检索   | 相关内容的排序质量       |
| Context Recall     | 检索   | 检索覆盖率               |
| Answer Correctness | 端到端 | 回答与标准答案匹配度     |

#### 2.4.3 双 LLM 引擎设计

评估 LLM 与业务 LLM 独立，支持两种模式：

| 模式     | Provider            | 适用场景             | 成本           |
| -------- | ------------------- | -------------------- | -------------- |
| 本地模式 | Ollama (qwen2.5:7b) | 离线评估、数据隐私   | 免费（需显卡） |
| 线上模式 | OpenAI / DashScope  | 高质量评估、快速迭代 | 按 token 计费  |

```python
# 工厂方法切换评估引擎
eval_llm = get_eval_llm(provider="ollama")       # 本地模式
eval_llm = get_eval_llm(provider="openai",        # 线上模式
                         model="gpt-4o-mini",
                         api_key="sk-...")
```
...
│   评估数据集 (JSON)                                          │
│   ┌─────────────────────────────────────────────────────┐   │
│   │ {question, ground_truth, kb_id} × N 条               │   │
│   └─────────────────────┬───────────────────────────────┘   │
│                         │                                    │
│                         ▼                                    │
│   ┌─────────────────────────────────────────────────────┐   │
│   │ 1. ChatService.chat(question, kb_id)                 │   │
│   │    → answer (LLM生成), contexts (检索结果)            │   │
│   └─────────────────────┬───────────────────────────────┘   │
│                         │                                    │
│                         ▼                                    │
│   ┌─────────────────────────────────────────────────────┐   │
...
│                         ▼                                    │
│   ┌─────────────────────────────────────────────────────┐   │
│   │ 3. ragas.evaluate(dataset, metrics, llm, embeddings) │   │
│   │    → 各指标分数 (0.0 ~ 1.0)                          │   │
│   └─────────────────────┬───────────────────────────────┘   │
│                         │                                    │
│                         ▼                                    │
...
│   │   ├── ollama_engine.py          # Ollama LLM 引擎
│   │   ├── chat_service.py           # 问答服务
│   │   ├── evaluator.py              # Ragas 评估器
│   │   └── langchain_adapters.py     # LangChain 适配层
│   │
│   ├── models/                       # 数据模型
│   │   ├── __init__.py
│   │   ├── database.py               # 数据库初始化
