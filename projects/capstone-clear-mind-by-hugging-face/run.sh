#!/bin/bash
# ============================================================
# ClearMind-HF — 一键训练脚本 (HuggingFace 生态版)
# ============================================================
# 使用方法: bash run.sh
# ============================================================
...
echo ""
echo -e "${BOLD}  ============================================================${NC}"
echo -e "${BOLD}    🤗 ClearMind-HF — 用 HuggingFace 生态训练你的大语言模型${NC}"
echo -e "${BOLD}  ============================================================${NC}"
...
echo -e "  ${BOLD}7)${NC} LoRA 微调  (需已有预训练模型)"
echo -e "  ${BOLD}8)${NC} 运行测试  pytest"
echo -e "  ${BOLD}9)${NC} 冒烟测试  数据 → 分词器 → 预训练(1步, tiny)"
echo ""
read -p "  请选择 [1-9, 默认 1]: " flow_choice
FLOW="${flow_choice:-1}"
...
case $FLOW in
    ...
    6)  # 仅训练分词器
        ...
        ;;
    7)  # LoRA 微调
        run_step "LoRA 微调"         "python scripts/train.py --stage sft --config $CONFIG --use_lora"
        ;;
    8)  # 运行测试
        run_step "运行单元测试"      "python -m pytest tests/ -v || (echo '  ⚠️ pytest 插件冲突, 自动回退兼容模式' && PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q tests)"
        ;;
    9)  # 冒烟测试
        run_step "运行端到端冒烟测试" "python scripts/smoke_test.py --config configs/tiny.yaml --max_steps 1"
        ;;
    *)
        echo -e "${RED}  ❌ 无效选择${NC}"; exit 1
        ;;
