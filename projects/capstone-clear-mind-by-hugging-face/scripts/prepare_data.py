...
def create_sample_pretrain_data(output_path: str, n_samples: int = 5000):
    ...
    print(f"  pretrain data: {n_samples:,} samples")
...
def create_sample_sft_data(output_path: str, n_samples: int = 2000):
    ...
    print(f"  SFT data: {n_samples:,} samples")
...
def create_sample_dpo_data(output_path: str, n_samples: int = 500):
    ...
    print(f"  DPO data: {n_samples:,} samples")
...
def download_pretrain_data(ds_configs: list, output_path: str):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for ds_config in ds_configs:
            ...
            print(f"\n  Downloading: {desc} (target {max_samples:,} samples)")
    ...
    print(f"  Pretrain data total: {total_count:,} samples, {size_mb:.1f} MB")
...
def download_sft_data(ds_configs: list, output_path: str):
    ...
    size_mb = os.path.getsize(output_path) / 1024 / 1024
    print(f"  SFT data total: {total_count:,} samples, {size_mb:.1f} MB")
...
def download_dpo_data(ds_configs: list, output_path: str):
    ...
    size_mb = os.path.getsize(output_path) / 1024 / 1024
    print(f"  DPO data total: {total_count:,} samples, {size_mb:.1f} MB")
...
def create_tokenizer_corpus(data_dir: str):
    ...
    print(f"  Tokenizer corpus: {len(texts):,} lines")
...
def prepare_stage(stage: str, scale: str, data_dir: str):
    ...
    if use_sample or sample_count is not None:
        ...
        if not HAS_DATASETS:
            print(f"  {scale} scale requires datasets library: pip install datasets")
            sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="ClearMind-HF Data Preparation")
    parser.add_argument(
        "--scale",
        type=str,
        default="small",
        choices=["small", "medium", "large"],
        help="Data scale: small=sample, medium=100k, large=500k",
    )
    parser.add_argument(
        "--stage",
        type=str,
        ...
        help="Prepare data for specific stage only",
    )
    parser.add_argument("--data_dir", type=str, default="data", help="Data directory")
    args = parser.parse_args()

    print("=" * 60)
    print(f"ClearMind-HF Data Preparation (scale: {args.scale})")
    print("=" * 60)
    ...
    for i, stage in enumerate(stages, 1):
        ...
        print(f"\nStep {i}/{total}: {stage} data")
        prepare_stage(stage, args.scale, args.data_dir)

    # 分词器语料
    if args.stage == "all":
        print(f"\nStep {len(stages) + 1}/{len(stages) + 1}: tokenizer corpus")
    ...
    print(f"\nNext step: python scripts/train_tokenizer.py --config configs/tiny.yaml")
    print("=" * 60)
